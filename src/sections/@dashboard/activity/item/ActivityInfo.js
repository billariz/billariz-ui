...
import { EventList, JournalList } from 'src/sections/@dashboard/event/item';
...
export default function ActivityInfo({ open, onClose, activity }) {
  ...
  const items = useMemo(
    () => [
      {
        label: 'TableTabs.ActivityDetails',
  ...
  const renderTable = () => {
    switch (currentTable) {
      case 'activityDetails':
        return <EventList activity={activity} />;
      case 'journal':
        return <JournalList objectId={activity.id} objectType={'ACTIVITY'} />;
    }
  };
  return (
    <Dialog
      ...
      scrollbuttons="auto"
    >
      <Can I="READ" a={'ACTIVITY'} ability={ability} passThrough>
        {(allowed) =>
          allowed ? (
            <Container
              sx={{ py: 2, px: 0 }}
            ...
            >
              ...
              <Card>
                {activity && filteredItems.length > 0 && (
                  <Card>
                    <TableTabs
                      items={filteredItems}
