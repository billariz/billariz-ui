...
import {
  findServiceStartOptionsApi,
  addServiceStartOptionApi,
  deleteServiceStartOptionApi,
  updateServiceStartOptionApi,
} from 'src/api/serviceStartOptions';
import { ServiceStartOptionModel } from 'src/models/ServiceStartOption.model';
...
const slice = createSlice({
  ...
  reducers: {
    ...
    getServiceStartOptionSuccess(state, action) {
      state.isLoading = false;
      const apiResponse = action.payload;
      state.serviceStartOptions = apiResponse._embedded.serviceStartOptions.map(
        (serviceStartOption) => new ServiceStartOptionModel(serviceStartOption)
      );
    ...
    refreshData(state) {
      dispatch(getServiceStartOptions(...state.latestQuery));
    },
  },
});
...
export function getServiceStartOptions(page, size, arrayFilters) {
  return async (dispatch, getState) => {
    dispatch(slice.actions.hasError(null));
    dispatch(slice.actions.startLoading());
    try {
      if (!Number.isFinite(page) && !arrayFilters) {
        [page, size, arrayFilters] = getState().serviceStartOptions.latestQuery;
      }
      let params = {
        page,
        size,
      ...
      const apiResponse = await findServiceStartOptionsApi(params);
      dispatch(slice.actions.getServiceStartOptionSuccess(apiResponse));
    } catch (error) {
...
export function dispatchServiceStartOption(action, args, id) {
  return async () => {
    try {
      switch (action) {
        case 'create': {
          ...
          if (Array.isArray(args)) {
            for (const item of args) {
              const apiResponse = await addServiceStartOptionApi(
        ...
        case 'edit': {
          ...
          const resultUpdate = await updateServiceStartOptionApi(
            id,
        ...
        case 'delete': {
          const resultDelete = await deleteServiceStartOptionApi(id);
          if (!resultDelete) {
            dispatch(
