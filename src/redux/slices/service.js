...
const slice = createSlice({
  ...
  reducers: {
    ...
    getServicesSuccess(state, action) {
      ...
      state.services = apiResponse._embedded.services.map(
    ...
    refreshListByItem(state, action) {
      ...
      const updatedService = new ServiceModel(
        apiResponse._embedded.services[0]
      );
      ...
      if (index !== -1) {
        ...
        state.services[0] = updatedService;
      }
    },
...
export function getServices(page, size, arrayFilters) {
  return async (dispatch, getState) => {
    dispatch(slice.actions.hasError(null));
    dispatch(slice.actions.startLoading());
    try {
      if (!Number.isFinite(page) && !arrayFilters) {
        [page, size, arrayFilters] = getState().services.latestQuery;
      }
      let params = {
        page,
        size,
      ...
      const apiResponse = await findServicesApi(params);
      dispatch(slice.actions.getServicesSuccess(apiResponse));
    } catch (error) {
...
export function getServiceById(id) {
  return async (dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      const apiResponse = await findServicesApi({ id });
...
export function dispatchServiceAction(action, args, id, impliedValues = {}) {
  return async () => {
    try {
      switch (action) {
        case 'create': {
        ...
        case 'edit': {
          const payload = args;
          const resultUpdate = await updateServiceApi(id, payload);
          if (!resultUpdate) {
            dispatch(
        ...
        case 'delete': {
          const resultDelete = await deleteServiceApi(id);
          if (!resultDelete) {
            dispatch(
