...
import {
  ...
  findRateTypesApi,
  updateRateTypeApi,
} from 'src/api/rateTypes';
import { RateTypeModel } from 'src/models/RateType.model';
...
const slice = createSlice({
  ...
  reducers: {
    ...
    getRateTypesSuccess(state, action) {
      state.isLoading = false;
      const apiResponse = action.payload;
      state.rateTypes = apiResponse._embedded.rateTypes.map(
        (rateType) => new RateTypeModel(rateType)
      );
...
export function getRateTypes(page, size, arrayFilters) {
  return async (dispatch, getState) => {
    dispatch(slice.actions.hasError(null));
    dispatch(slice.actions.startLoading());
    try {
      if (!Number.isFinite(page) && !arrayFilters) {
        [page, size, arrayFilters] = getState().rateTypes.latestQuery;
      }
      let params = {
        page,
        size,
      ...
      const apiResponse = await findRateTypesApi(params);
      dispatch(slice.actions.getRateTypesSuccess(apiResponse));
    } catch (error) {
...
export function dispatchRateTypeAction(action, args, id) {
  return async () => {
    try {
      switch (action) {
        case 'create': {
          ...
          if (Array.isArray(args)) {
            for (const item of args) {
              const apiResponse = await addRateTypeApi(item);
        ...
        case 'edit': {
          ...
          const resultUpdate = await updateRateTypeApi(id, payload);
          if (!resultUpdate) {
            dispatch(
        ...
        case 'delete': {
          const resultDelete = await deleteRateTypeApi(id);
          if (!resultDelete) {
            dispatch(
