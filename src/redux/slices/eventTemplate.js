...
import { EventTemplateModel } from 'src/models/EventTemplate.model';
import {
  updateEventTemplateApi,
  addEventTemplateApi,
  deleteEventTemplateApi,
  findEventTemplateApi,
} from 'src/api/eventTemplate';
...
const slice = createSlice({
  ...
  reducers: {
    ...
    getEventTemplatesSuccess(state, action) {
      state.isLoading = false;
      const apiResponse = action.payload;
      state.eventTemplate = apiResponse._embedded.eventTemplates.map(
        (eventTemplate) => new EventTemplateModel(eventTemplate)
      );
    ...
    refreshData(state) {
      dispatch(getEventTemplate(...state.latestQuery));
    },
...
export function getEventTemplate(page, size, arrayFilters) {
  return async (dispatch, getState) => {
    dispatch(slice.actions.hasError(null));
    dispatch(slice.actions.startLoading());
    try {
      if (!Number.isFinite(page) && !arrayFilters) {
        [page, size, arrayFilters] = getState().eventTemplate.latestQuery;
      }
      ...
      const apiResponse = await findEventTemplateApi(params);
      dispatch(slice.actions.getEventTemplatesSuccess(apiResponse));
    } catch (error) {
...
export function dispatchEventTemplate(action, args, id) {
  return async () => {
    try {
      switch (action) {
        case 'create': {
          ...
          if (Array.isArray(args)) {
            for (const item of args) {
              const apiResponse = await addEventTemplateApi(item);
        ...
        case 'edit': {
          ...
          const resultUpdate = await updateEventTemplateApi(id, payload);
          if (!resultUpdate) {
            dispatch(
        ...
        case 'delete': {
          const resultDelete = await deleteEventTemplateApi(id);
          if (!resultDelete) {
            dispatch(
