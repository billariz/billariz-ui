...
import { MeterModel } from 'src/models/Meter.model';
...
const slice = createSlice({
  ...
  reducers: {
    ...
    getMeterSuccess(state, action) {
      state.isLoading = false;
      const apiResponse = action.payload;
      state.meters = apiResponse._embedded.meters.map(
        (meter) => new MeterModel(meter)
      );
    ...
    refreshData(state) {
      dispatch(getMeter(...state.latestQuery));
    },
...
export function getMeter(page, size, arrayFilters) {
  return async (dispatch, getState) => {
    dispatch(slice.actions.hasError(null));
    dispatch(slice.actions.startLoading());
    try {
      if (!Number.isFinite(page) && !arrayFilters) {
        [page, size, arrayFilters] = getState().meters.latestQuery;
      }
      let params = {
        page,
        size,
      ...
      const apiResponse = await findMeterApi(params);
      dispatch(slice.actions.getMeterSuccess(apiResponse));
    } catch (error) {
...
export function dispatchMeter(action, args, id, impliedValues = {}) {
  return async () => {
    try {
      switch (action) {
        case 'duplicate':
        case 'create':
          {
            ...
            if (Array.isArray(args)) {
              for (const item of args) {
                const apiResponse = await addMeterApi({
                  ...impliedValues,
                  ...item,
                });
        ...
        case 'edit': {
          ...
          const resultUpdate = await updateMeterApi(id, payload);
          if (!resultUpdate) {
            dispatch(
        ...
        case 'delete': {
          const resultDelete = await deleteMeterApi(id);
          if (!resultDelete) {
            dispatch(
