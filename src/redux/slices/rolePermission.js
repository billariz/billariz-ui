...
const slice = createSlice({
  ...
  reducers: {
    ...
    getRolePermissionsSuccess(state, action) {
      ...
      state.permissions = _embedded.rolePermissions.map(
        (rp) => new RolePermissionModel(rp)
      );
...
export function getRolePermissions(page, size, arrayFilters) {
  return async (dispatch, getState) => {
    ...
    try {
      ...
      if (!Number.isFinite(page) && !arrayFilters) {
        [page, size, arrayFilters] = getState().rolePermissions.latestQuery;
      }
      ...
      // Fetch API call
      const apiResponse = await findRolePermissionsApi(params);
      dispatch(slice.actions.getRolePermissionsSuccess(apiResponse));
    } catch (error) {
...
export function dispatchRolePermissionAction(action, args, id, impliedValues) {
  return async () => {
    try {
      let result;
      switch (action) {
        case 'create': {
          result = Array.isArray(args)
            ? await Promise.all(
                args.map((item) =>
                  addRolePermissionApi({ ...item, ...impliedValues })
                )
              )
            : [
                await addRolePermissionApi({
                  ...item,
                  ...impliedValues,
                }),
              ];
        ...
        case 'edit': {
          const updateResponse = await updateRolePermissionApi(id, {
        ...
        case 'delete': {
          const deleteResponse = await deleteRolePermissionApi(id);
          if (!deleteResponse) {
            dispatch(
