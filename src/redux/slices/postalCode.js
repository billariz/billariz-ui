...
import {
  ...
  findPostalCodesApi,
  updatePostalCodeApi,
} from 'src/api/postalCodes';
import { dispatch } from '../store';
import { PostalCodeModel } from 'src/models/PostalCode.model';
...
const slice = createSlice({
  ...
  reducers: {
    ...
    getPostalCodesSuccess(state, action) {
      state.isLoading = false;
      const { _embedded, page } = action.payload;
      state.postalCodes = _embedded.postalCodes.map(
        (postalCode) => new PostalCodeModel(postalCode)
      );
    ...
    // REFRESH DATA
    refreshData(state) {
      dispatch(getPostalCodes(...state.latestQuery));
    },
...
// Fetch postal codes with filters
export function getPostalCodes(page, size, arrayFilters) {
  return async (dispatch, getState) => {
    ...
    try {
      ...
      if (!Number.isFinite(page) && !arrayFilters) {
        [page, size, arrayFilters] = getState().postalCodes.latestQuery;
      }
      ...
      // Fetch API call
      const apiResponse = await findPostalCodesApi(params);
      dispatch(slice.actions.getPostalCodesSuccess(apiResponse));
    } catch (error) {
...
// Dispatch postal code-related actions (create, edit, delete)
export function dispatchPostalCodeAction(action, args, id) {
  return async () => {
    try {
      let result;
      switch (action) {
        case 'create': {
          result = Array.isArray(args)
            ? await Promise.all(args.map((item) => addPostalCodeApi(item)))
            : [await addPostalCodeApi(args)];
        ...
        case 'edit': {
          const updateResponse = await updatePostalCodeApi(id, args[0]);
          if (!updateResponse) {
            dispatch(
        ...
        case 'delete': {
          const deleteResponse = await deletePostalCodeApi(id);
          if (!deleteResponse) {
            dispatch(
