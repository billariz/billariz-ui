...
import {
  ...
  deleteTosTypeApi,
  updateTosTypeApi,
} from 'src/api/tosTypes';
import { TosTypeModel } from 'src/models/TosType.model';
...
const slice = createSlice({
  ...
  reducers: {
    ...
    getTosTypesSuccess(state, action) {
      state.isLoading = false;
      const apiResponse = action.payload;
      state.tosTypes = apiResponse._embedded.termOfServiceTypes.map(
        (tosType) => new TosTypeModel(tosType)
      );
    ...
    refreshData(state) {
      dispatch(getTosTypes(...state.latestQuery));
    },
  },
});
...
export function getTosTypes(page, size, arrayFilters) {
  return async (dispatch, getState) => {
    dispatch(slice.actions.hasError(null));
    dispatch(slice.actions.startLoading());
    try {
      if (!Number.isFinite(page) && !arrayFilters) {
        [page, size, arrayFilters] = getState().tosTypes.latestQuery;
      }
      let params = {
        page,
        size,
      ...
      const apiResponse = await findTosTypesApi(params);
      dispatch(slice.actions.getTosTypesSuccess(apiResponse));
    } catch (error) {
...
export function dispatchTosType(action, args, id) {
  return async () => {
    try {
      switch (action) {
        case 'create': {
          ...
          if (Array.isArray(args)) {
            for (const item of args) {
              const apiResponse = await addTosTypeApi(
                transformEmptyToNull(item)
              );
              result.push(apiResponse);
            }
          } else {
            const apiResponse = await addTosTypeApi(transformEmptyToNull(item));
            result.push(apiResponse);
          }
        ...
        case 'edit': {
          ...
          const resultUpdate = await updateTosTypeApi(id, payload);
          if (!resultUpdate) {
            dispatch(
        ...
        case 'delete': {
          const resultDelete = await deleteTosTypeApi(id);
          if (!resultDelete) {
            dispatch(
